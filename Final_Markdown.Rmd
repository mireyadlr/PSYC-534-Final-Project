...
First, we created two functions in order to run the analyses correctly using **function()**. The first function creates a vector for the regions and the specific variable we are testing.The second function gets the p-values from the t-tests we ran. 
```{r Analyses}
...
#gets p-values from the t-tests

```

So we have the p-values from the t-tests but we cannot see it in an organized fashion so we needed to create a table of the p-values in order to see them and known which p-value is from which t-test. First, we created a vector of regions so that they are each assigned a number by row number. We, then, created an empty matrix that will hold the later p-values we will get using **matrix()** and gave the columns and rows their correct region label. Then, we finally put the p-values into the matrix by row and then by column using a *for loop*. 
...
#Gave the matrix's columns and rows their correct headers
...
#Finally, we view matrix (specifically for GDP variable)
#View(pMatrix)
```
